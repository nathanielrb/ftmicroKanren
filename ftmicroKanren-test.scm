...
(newline)
(print "Original microKanren tests")
...
(test-check "conj with one later"
  (promise? ((call/fresh (lambda (q) (conj (== q 4) (next (== q 5))))) empty-state))
  #t)
...
(test-check "basic functioning of disj"
 (current ((disj (next (== #(0) 6)) (disj (== #(0) 5) (next (== #(0) 7)))) empty-state))
 (current ((disj (== #(0) 5) (next (disj (== #(0) 6) (== #(0) 7)))) empty-state)))
...
(test-check "nested promises, promised"
  (current (force (promised $1)))
  '(4))

(test-check "nested promises, twice promised"
  (force (promised (force (promised $1))))
  '())
...
(test-check "recursively nested promises, first promise"
  (current (force (promised $2)))
  '((0 1) (1 0) (1 1)))

(test-check "recursively nested promises, second promise"
  (current (force (promised (force (promised $2)))))
  '((1 2) (2 0) (2 1) (2 2) (0 2)))

(test-check "recursively nested promises, third promise"
  (current (force (promised (force (promised (force (promised $2)))))))
